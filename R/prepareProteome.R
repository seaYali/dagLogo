...
#' If both are specified, taxonID will be used preferentially.
...
#' with different strains, attention should be paid.
        ...
        if (!is.null(species) && is.null(taxonID))
        {
            ...
            if ( numberOfEntries != 1) 
            {
                ...
                } else 
                {
                    ...
                    {
                        stop("There are multiple entries in the Uniprot database matching your input species. ", "Please specify the exact one by referring to the the NCBI taxonomy database: https://www.ncbi.nlm.nih.gov/taxonomy or the UniProt database http://www.uniprot.org/taxonomy.")
                    }
                }
            } else 
            {
