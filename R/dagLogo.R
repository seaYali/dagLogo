...
colorsets <-function(colorScheme = c("null", "classic", "charge", "chemistry", "hydrophobicity")) 
{
    ...
    switch(colorScheme,
           null = get("auto", envir = cacheEnv),
           classic = get("classic", envir = cacheEnv)$color,
           charge = get("charge", envir = cacheEnv)$color,
           chemistry = get("chemistry", envir = cacheEnv)$color,
           hydrophobicity = get("hydrophobicity", envir = cacheEnv)$color,
           get("auto", envir = cacheEnv)) ##default
}
...
nameHash <-function(nameScheme = c("classic", "charge", "chemistry", "hydrophobicity"))
{
        ...
        switch(
            nameScheme,
            classic = get("classic", envir = cacheEnv)$symbol,
            charge = get("charge", envir = cacheEnv)$symbol,
            chemistry = get("chemistry", envir = cacheEnv)$symbol,
            hydrophobicity = get("hydrophobicity", envir = cacheEnv)$symbol,
        )
}
                    ...
                    labels = NULL) 
{
    ...
    key <- paste("x",
                 ...
                 sep = "_")
    symbolsCache <-
        if (exists("tmp_motifStack_symbolsCache", envir = cacheEnv))
            get("tmp_motifStack_symbolsCache", envir = cacheEnv)
        else
            list()
    ...
    } else 
    {
        ...
        assign("tmp_motifStack_symbolsCache", symbolsCache, envir = cacheEnv)
    }
    ...
    datP <- apply(dat, 2, function(.col)
        sum(.col[.col > 0]))
    ylim <-
        c((as.integer(min(datN) / 0.05) - 1) * 0.05, (as.integer(max(datP) / 0.05) +
                                                          1) * 0.05)
    remap <- function(x) {
    ...
    ##draw tick
    tick <-
        ifelse(type == "diff", 0.1, 10 ^ as.integer(log10(max(
            abs(as.numeric(dat))
        ))))
    times <- ifelse(type == "diff", 100, 1)
    for (i in c(as.integer(min(datN) / tick):(-1), 1:as.integer(max(datP) /
                                                                tick))) {
        grid.lines(
            x = c(x2, x0),
            y = remap(i * tick) + dw / 2,
    ...
    if (legend) 
    {
        if (is.null(namehash)) 
        {
            namehash <- get("namehash", envir = cacheEnv)
        }
